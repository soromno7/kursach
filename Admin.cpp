...
const char* baseOfPatientsFile = "Books.txt";
const char* Admin::baseOfUsersFile = "Admins.txt";
...
void Admin::redactRecord() { //функция читает все объекты из файла, и изменяет поле одного из объектов на выбор пользователя, 
	...
	cout << endl << "Какое поле необходимо изменить?"
		<< endl << "1. Название компании-поставщика"
		<< endl << "2. Количество поставленных книг"
		<< endl << "3. Дату поставки книг"
		<< endl << "4. Время поставки книг"
		<< endl << "5. Название книги"
		<< endl << "6. Автор книги" << endl;
	cin >> choiceOfField;
	if (std::cin.fail()) // если предыдущее извлечение оказалось неудачным,
	{
		std::cin.clear(); // то возвращаем cin в 'обычный' режим работы
	...
	case 1: records[choiceOfRecord - 1].setpName(newData);
		break;
	case 2: records[choiceOfRecord - 1].setkbook(newData);
		break;
	case 3: records[choiceOfRecord - 1].setdate(newData);
		break;
	case 4: records[choiceOfRecord - 1].settime(newData);
		break;
	case 5: records[choiceOfRecord - 1].setnameOfBook(newData);
		break;
	case 6: records[choiceOfRecord - 1].setauthor(newData);
		break;
...
void Admin::searchRecord() {  //метод ищет записи по тому, что запишет пользователь, используются регулярные выражения
	...
	switch (choiceOfField) {
	case 1:
		getSomeData = &Record::getpName;  //принимает указатель на метод, по которому потом будем искать совпадения
		break;
	case 2:
		getSomeData = &Record::getkbook;
		break;
	case 3:
	...
	case 4:
		...
		break;
	case 5:
		getSomeData = &Record::getnameOfBook;
		break;
	case 6:
		getSomeData = &Record::getauthor;
		break;
	default: cout << endl << "Такого поля не существует";
		return;
		break;
...
void Admin::filterRecords() { //фильрация оставляет в базе только записи, моложе заданной даты(тоесть удаляет устаревшие)
	...
	cout << "Введите дату поставки" << std::endl;
	cin.get();
